...
def init_config():
    ...
    #arg_parser.add_argument('--two-factor-auth', dest='two_factor_auth', help="Two factor authentication code.")
    arg_parser.add_argument('-f', '--two-factor-auth', dest='two_factor_auth', help="Two factor authentication code.")
    ...
    #arg_parser.add_argument('--ignore-projects',  dest='ignore_projects',  action='store_true', help="Do not import projects attached to the issue.")
    ...
    include_group.add_argument("-i", "--issues", dest='issues', help="The list of issues to import.");
    ...
    if args.two_factor_auth: config.set('login', 'two_factor_auth', args.two_factor_auth)
    ...
    #config.set('settings', 'import-projects',  str(not args.ignore_projects))
    ...
    def get_credentials_for(which):
        ...
        if not config.has_option(which, 'two_factor_auth'):
            if config.has_option('login', 'two_factor_auth'):
                config.set(which, 'two_factor_auth', config.get('login', 'two_factor_auth'))
    ...
    try:
        ...
        for issue in issues:
            if '-' in issue:
                issue_range = [int(i) for i in issue.split('-')]
                for i in range(issue_range[0], issue_range[1] + 1):
                    issue_nums.add(int(i))
            else:
                issue_nums.add(int(issue))
    print(issue_nums)
    return issue_nums
...
def send_request(which, url, post_data=None):
    ...
    if url == 'projects':
        req.add_header("Accept", "application/vnd.github.inertia-preview")
    else:
        req.add_header("Accept", "application/json")

    try:
        two_factor_auth = config.get(which, 'two_factor_auth')
        req.add_header("X-GitHub-OTP", two_factor_auth) 
...
def get_projects(which):
    return send_request(which, "projects")
    
# get project columns
# curl -i https://api.github.com/projects/1709161/columns -u Janulka -H "Accept:   application/vnd.github.inertia-preview+json"
# curl -i https://api.github.com/projects/<project-ID>/columns -u Janulka -H "Accept:   application/vnd.github.inertia-preview+json"
...
def import_issues(issues):
    ...
    for issue in issues:
        ...
        if config.getboolean('settings', 'import-milestone') and 'milestone' in issue and issue['milestone'] is not None:
            ...
            print('import_milestone')
            print(found_milestone)
            if found_milestone:
        ...
        #TODO move and assign issue projects 
